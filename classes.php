...
class Questions{
    ...
    public function add_question($eventId,$question,$a,$b,$c,$d,$e,$correct,$image,$type,$keywords,$userID){
        ...
        if($type == 1){
            ...
            Values(?,?,?,?,?,?,?,?,?,?,?,?)";
            $add = $dbh->prepare($sql);
            $add->execute(array($eventId,$totalMax,$question,$a,$b,$c,$d,$e,$correct,$type,$mysqltime,$userID));
        }elseif($type ==3){
            ...
            $add = $dbh->prepare($sql);
            $add->execute(array($eventId,$totalMax,$question,$a,$b,$c,$d,$e,$correct,$type,$image,$mysqltime,$userID));
        }elseif($type ==4){
            ...
            $add = $dbh->prepare($sql);
            $add->execute(array($eventId,$totalMax,$question,$keywords,$type,$image,$mysqltime,$userID));
        }else{
            $question = $this->check_delim($question);
            if(!$this->check_single_delim($question)){
                //if there is more then one deliminator break.
            ...
            $add = $dbh->prepare($sql);
            $add->execute(array($eventId,$totalMax,$question,$type,$keywords,$mysqltime,$userID));
        }
